...
- **Alert rules** — watch for query-volume spikes (per-client or overall), new
  ...
  keywords, a **first-seen domain** (the domain-keyed sibling of "new device"/
  "new vendor" — fires when a domain is queried that no client has ever queried
  before, network-wide, not just new to one device), or a **device going
